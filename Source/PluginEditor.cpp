...
// Destructor
// ==========================================================================
AetherAudioProcessorEditor::~AetherAudioProcessorEditor() { saveWindowSize(); }

// ==========================================================================
// Painting & Layout
// ==========================================================================
void AetherAudioProcessorEditor::paint(juce::Graphics &g) {
  // Draw solid very dark background matching the CSS theme in case of slow
  // loading
...
void AetherAudioProcessorEditor::timerCallback() {
  ...
  if (editSnap != webView.localActiveSnapshot) {
    ...
    webView.evaluateJavascript(
        "if (window.aetherUI) "
        "window.aetherUI.updateParamFromCpp('editSnapshot', " +
        juce::String(editSnap + 1) + ");");
    ...
    for (int i = 0; i < 15; ++i) {
      ...
      webView.localSteps[i].muted =
          !audioProcessor.snapshots[editSnap].steps[i].muted;
    }
  }

  // 2. Push per-snapshot scalar parameters.
  //    Read directly from snapshots[editSnap] — the authoritative per-snapshot
  //    store.
  ...
  // enabled
  {
    ...
    if (std::abs(val - webView.localParams[0]) > 0.001f) {
      ...
      webView.evaluateJavascript(
          "if (window.aetherUI) "
          "window.aetherUI.updateParamFromCpp('enabled', " +
          juce::String(snap.enabled ? "true" : "false") + ");");
    }
  }
  // delayTimeMs
  {
    ...
    if (std::abs(val - webView.localParams[1]) > 0.001f) {
      ...
      AetherWebView::logToFile("timer pushing: delayTimeMs = " +
                               juce::String(val));
      webView.evaluateJavascript(
          "if (window.aetherUI) "
          "window.aetherUI.updateParamFromCpp('delayTimeMs', " +
          juce::String(val) + ");");
    }
  }
  // syncDivision
  {
    ...
    if (std::abs(val - webView.localParams[2]) > 0.001f) {
      ...
      AetherWebView::logToFile("timer pushing: syncDivision = " +
                               juce::String(snap.syncDivision));
      webView.evaluateJavascript(
          "if (window.aetherUI) "
          "window.aetherUI.updateParamFromCpp('syncDivision', " +
          juce::String(snap.syncDivision) + ");");
    }
  }
  // stepCount
  {
    ...
    if (std::abs(val - webView.localParams[3]) > 0.001f) {
      ...
      AetherWebView::logToFile("timer pushing: stepCount = " +
                               juce::String(snap.stepCount));
      webView.evaluateJavascript(
          "if (window.aetherUI) "
          "window.aetherUI.updateParamFromCpp('stepCount', " +
          juce::String(snap.stepCount) + ");");
    }
  }
  // killOnStop (Global param — read from APVTS)
  {
    auto *kosP = dynamic_cast<juce::AudioParameterBool *>(
        audioProcessor.apvts.getParameter("killOnStop"));
    ...
    if (std::abs(val - webView.localParams[4]) > 0.001f) {
      ...
      AetherWebView::logToFile("timer pushing: killOnStop = " +
                               juce::String(val));
      webView.evaluateJavascript(
          "if (window.aetherUI) "
          "window.aetherUI.updateParamFromCpp('killOnStop', " +
          juce::String(kosBool ? "true" : "false") + ");");
    }
  }
  // activeSnapshot — single global param, correctly read from APVTS
  {
    auto *actP = dynamic_cast<juce::AudioParameterInt *>(
        audioProcessor.apvts.getParameter("activeSnapshot"));
    int actVal = actP ? actP->get() : 1;
    float val = (float)actVal;
    if (std::abs(val - webView.localParams[5]) > 0.001f) {
      webView.localParams[5] = val;
      AetherWebView::logToFile("timer pushing: activeSnapshot = " +
                               juce::String(actVal));
      webView.evaluateJavascript(
          "if (window.aetherUI) "
          "window.aetherUI.updateParamFromCpp('activeSnapshot', " +
          juce::String(actVal) + ");");
    }
  }
  // killOnSwitch (Global param — read from APVTS)
  {
    auto *kswP = dynamic_cast<juce::AudioParameterBool *>(
        audioProcessor.apvts.getParameter("killOnSwitch"));
    ...
    if (std::abs(val - webView.localParams[6]) > 0.001f) {
      ...
      AetherWebView::logToFile("timer pushing: killOnSwitch = " +
                               juce::String(val));
      webView.evaluateJavascript(
          "if (window.aetherUI) "
          "window.aetherUI.updateParamFromCpp('killOnSwitch', " +
          juce::String(kswBool ? "true" : "false") + ");");
    }
  }
  // endSwitch (Global param — read from APVTS)
  {
    auto *esP = dynamic_cast<juce::AudioParameterBool *>(
        audioProcessor.apvts.getParameter("endSwitch"));
    ...
    if (std::abs(val - webView.localParams[7]) > 0.001f) {
      ...
      AetherWebView::logToFile("timer pushing: endSwitch = " +
                               juce::String(val));
      webView.evaluateJavascript(
          "if (window.aetherUI) "
          "window.aetherUI.updateParamFromCpp('endSwitch', " +
          juce::String(esBool ? "true" : "false") + ");");
    }
  }
  // actualActiveSnapshot (from processor atomic)
  {
    ...
    if (std::abs(val - webView.localParams[8]) > 0.001f) {
      ...
      webView.evaluateJavascript(
          "if (window.aetherUI) window.aetherUI.setActualActiveSnapshot(" +
          juce::String(actVal) + ");");
    }
  }
  ...
  // modwheelSlew
  {
    ...
    if (std::abs(val - webView.localParams[9]) > 0.001f) {
      ...
      AetherWebView::logToFile("timer pushing: modwheelSlew = " +
                               juce::String(val));
      webView.evaluateJavascript(
          "if (window.aetherUI) "
          "window.aetherUI.updateParamFromCpp('modwheelSlew', " +
          juce::String(val) + ");");
    }
  }
  ...
  if (stepsChanged) {
    ...
    webView.evaluateJavascript(
        "if (window.aetherUI) window.aetherUI.updateStepsFromCpp('" +
        jsonSteps + "');");
  }
  ...
  if (hits > 0) {
    webView.evaluateJavascript(
        "if (window.aetherUI && window.aetherUI.triggerActivity) "
        "window.aetherUI.triggerActivity(" +
        juce::String(hits) + ");");
  }
}
...
juce::String AetherAudioProcessorEditor::getStepsJson() {
  ...
  for (int i = 0; i < 15; ++i) {
    ...
    stepObj->setProperty(
        "pitch", audioProcessor.snapshots[editSnap].steps[i].pitchOffset);
    stepObj->setProperty("velocity",
                         audioProcessor.snapshots[editSnap].steps[i].velocity);
    stepObj->setProperty("modwheel",
                         audioProcessor.snapshots[editSnap].steps[i].modwheel);
    stepObj->setProperty(
        "probability", audioProcessor.snapshots[editSnap].steps[i].probability);
    stepObj->setProperty("muted",
                         audioProcessor.snapshots[editSnap].steps[i].muted);
