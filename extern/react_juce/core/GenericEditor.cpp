...
//==============================================================================
GenericEditor::GenericEditor(juce::AudioProcessor &proc,
                             const juce::File &bundle)
    : juce::AudioProcessorEditor(proc),
      engine(std::make_shared<EcmascriptEngine>()), appRoot(engine),
      harness(appRoot) {
  // Sanity check
  ...
  for (auto &p : params) {
    ...
    if (auto paramWithID =
            dynamic_cast<juce::AudioProcessorParameterWithID *>(p)) {
  ...
  harness.onEvalError = [](const reactjuce::EcmascriptEngine::Error &e) {
    DBG("JS Error: " << e.what());
    DBG("JS Stack: " << e.stack);
  };
...
void GenericEditor::timerCallback() {
  ...
  for (size_t i = 0; i < paramReadouts.size(); ++i) {
    ...
    if (pr.dirty) {
      ...
      appRoot.dispatchEvent("parameterValueChange", static_cast<int>(i), id,
...
void GenericEditor::beforeBundleEvaluated() {
  engine->registerNativeMethod(
      ...
      [this](const juce::var::NativeFunctionArgs &args) {
        if (auto it = parameters.find(args.arguments[0].toString());
            it != parameters.cend())
